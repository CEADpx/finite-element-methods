...
![Resolution of a vector into components along an orthonormal Cartesian basis in $\R^2$.](../figures/chapter02/cartesian-basis-components.png){#fig-foundations-cartesian-basis width="72%" fig-align="center"}
...
Let $I=[0,1]$, define $u_n(x)=x^n$ for $x\in I$, and let
...
1. Show that $u_n(x)\to u(x)$ at every fixed $x\in I$.
2. Show that $\sup_{x\in I}|u_n(x)-u(x)|=1$ for every $n$.
...
::: {#exr-foundations-uniform-l2-convergence}
## Returning to $u_n(x)=x^n$
Let $I=[0,1]$, and recall the sequence and its pointwise limit $u$ from
@exr-foundations-pointwise-not-uniform.

1. Compute $\norm{u_n-u}_{L^2(I)}$ and determine whether $u_n\to u$ in
   $L^2(I)$.
2. Identify a constant function that represents the same element of $L^2(I)$
   as $u$, and explain why the two functions are equivalent in $L^2(I)$.
3. Compare this result with the failure of convergence in the supremum norm.
   What distinction between the two norms does this example illustrate?
:::

Vector- and tensor-valued $L^2$ spaces are defined componentwise. For example,

$$
\norm{\bu}_{L^2(\Om;\R^d)}^2
=\int_{\Om}\bu\cdot\bu\,\dx,
\qquad
\norm{\bm A}_{L^2(\Om;\R^{d\times d})}^2
=\int_{\Om}\bm A:\bm A\,\dx.
$$
...
In this section, $\Om\subset\R^d$ is open.
...
::: {#def-foundations-test-functions}
## Test functions
For $\varphi\in C^\infty(\Om)$, the **support** of $\varphi$ is

$$
\operatorname{supp}\varphi
=\overline{\{\bx\in\Om:\varphi(\bx)\ne0\}},
$$

where the closure is taken in $\R^d$. The space of test functions is

$$
C_0^\infty(\Om)
=\{\varphi\in C^\infty(\Om):
\operatorname{supp}\varphi\text{ is a compact subset of }\Om\}.
$$

Thus every $\varphi\in C_0^\infty(\Om)$ is infinitely differentiable and is
zero in a neighborhood of $\partial\Om$.
:::

::: {#thm-foundations-fundamental-lemma}
## Fundamental lemma for test functions
Let $q\in L^1_{\mathrm{loc}}(\Om)$. If

$$
\int_{\Om}q\varphi\,\dx=0
\qquad\forall\varphi\in C_0^\infty(\Om),
$$

then $q=0$ almost everywhere in $\Om$.
:::
...
The fundamental lemma @thm-foundations-fundamental-lemma shows that a weak
derivative, when it exists, is unique up to equality almost everywhere. For a
smooth function it agrees with the classical derivative. A function may have a
weak derivative even when its classical derivative fails at isolated points.
This enlarged notion of differentiation is essential for piecewise polynomial
finite element functions [@suli2012].
...
To verify @def-weak-derivative, let
$\varphi\in C_0^\infty(-1,1)$. Since $|x|=-x$ on $(-1,0)$ and
$|x|=x$ on $(0,1)$, split the integral at the origin and integrate by parts
on each subinterval:

$$
\begin{aligned}
-\int_{-1}^{1}|x|\varphi'(x)\,\dd x
&=\int_{-1}^{0}x\varphi'(x)\,\dd x
  -\int_{0}^{1}x\varphi'(x)\,\dd x\\
&=\left[x\varphi(x)\right]_{-1}^{0}
  -\int_{-1}^{0}\varphi(x)\,\dd x
...
The endpoint terms vanish because $\varphi(-1)=\varphi(1)=0$, and the two
terms evaluated at the origin contain the factor $x=0$. The defining identity
therefore holds for every $\varphi\in C_0^\infty(-1,1)$, so $w$ is the weak
derivative of $|x|$. The assigned value $w(0)=0$ is immaterial; changing a
function at one point does not change its equivalence class in $L^2(-1,1)$.
:::
...
$$
\seminorm{v}_{H^1(\Om)}
=\norm{\grad v}_{L^2(\Om;\R^d)}.
$$ {#eq-h1-seminorm}

The vector-valued space $H^1(\Om;\R^d)$ consists of fields whose $d$
components belong to $H^1(\Om)$. Its squared norm is the sum of the squared
$H^1$ norms of those components. We use this codomain notation consistently
for vector- and tensor-valued function spaces.
...
For vector-valued fields, $H^2(\Om;\R^d)$ is defined componentwise.
...
2. For an arbitrary $\varphi\in C_0^\infty(-1,1)$, split the integral at
   $x=0$ and verify

   $$
   \int_{-1}^1u(x)\varphi'(x)\,\dd x
   =-\int_{-1}^1w(x)\varphi(x)\,\dd x.
   $$

3. Use the definition of $H^1(-1,1)$ to determine whether
   $u\in H^1(-1,1)$.
4. Determine whether $u\in H^2(-1,1)$ by completing the following
   contradiction argument. If $w$ had a weak derivative
   $g\in L^2(-1,1)$, then

   $$
   \int_{-1}^1w(x)\varphi'(x)\,\dd x
   =-\int_{-1}^1g(x)\varphi(x)\,\dd x
   \qquad\forall\varphi\in C_0^\infty(-1,1).
   $$

   First show that the left-hand side equals $2\varphi(0)$. Next, choose
   $\eta\in C_0^\infty(-1,1)$ with $\eta(0)=1$ and set
   $\varphi_\varepsilon(x)=\eta(x/\varepsilon)$ for $0<\varepsilon<1$.
   Use

   $$
   \norm{\varphi_\varepsilon}_{L^2(-1,1)}
   =\sqrt{\varepsilon}\,\norm{\eta}_{L^2(-1,1)}
   $$

   and the Cauchy--Schwarz inequality to obtain a contradiction as
   $\varepsilon\to0$. Conclude whether $w\in H^1(-1,1)$ and therefore
   whether $u\in H^2(-1,1)$.
:::
...
$\bm{v}\in V=H^1(\Om;\R^d)$ be a displacement field. A body force
$\bm{b}\in L^2(\Om;\R^d)$ defines the functional
...
traction $\bm{t}\in L^2(\Gamma_t;\R^d)$ is prescribed. The field $\bm{v}$ has a
...
$$
\norm{\bm{v}}_{L^2(\Gamma_t;\R^d)}
\leq C_{\mathrm{tr}}\norm{\bm{v}}_{H^1(\Om;\R^d)}
$$
...
\leq
\left(
\norm{\bm{b}}_{L^2(\Om;\R^d)}
+C_{\mathrm{tr}}\norm{\bm{t}}_{L^2(\Gamma_t;\R^d)}
\right)
\norm{\bm{v}}_{H^1(\Om;\R^d)}.
$$
...
Use the Cauchy--Schwarz and Poincare inequalities to prove
