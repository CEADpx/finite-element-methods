...
In the equations above, $\Delta$ is the Laplacian and is defined by
$\Delta u(\bx)=\nabla\cdot(\nabla u(\bx))$.
...
To interpret this problem classically, suppose that $\partial\Om$ is of class
$C^1$, so that the outward unit normal $\bn$ is defined pointwise on
$\partial\Om$. Let

$$
k\in C^1(\Om)\cap C(\overline{\Om}),
\qquad
k(\bx)\geq k_0>0,
\qquad
f\in C(\Om),
...
$$
g\in C(\overline{\Gam_D}),
\qquad
h\in C(\overline{\Gam_N}),
$$

and suppose that a solution
$u\in C^2(\Om)\cap C^1(\overline{\Om})$ exists. Then
$\divg(k\grad u)$ is defined pointwise in $\Om$, while $u$ and
$k\grad u\cdot\bn$ are defined pointwise on the corresponding portions of the
boundary. Hence @eq-variational-strong-pde and
@eq-variational-boundary-data can be imposed in the classical sense.
...
$L^2(\Om;\R^d)$. Combining this requirement with $u,v\in L^2(\Om)$ gives
...
Such a functional need not exist. For example, let $a$ be a nonsymmetric
bilinear form and define $J(u):=(1/2)a(u,u)$. Its first variation is
