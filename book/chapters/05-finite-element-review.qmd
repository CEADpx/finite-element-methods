...
The inverse connectivity map $J^e$ is defined only for the global nodes that
belong to element $e$:

$$
J^e:\{I^e(1),I^e(2)\}\longrightarrow\{1,2\},
\qquad
J^e\bigl(I^e(a)\bigr)=a.
$$ {#eq-c5-inverse-connectivity}

Thus $J^e(i)$ gives the local number of global node $i$ on element $e$. For
the shared node $x_2$ in @fig-c5-local-global-numbering,
...
The element superscript is necessary because the same global node can have a
different local number in each incident element.
...
\mathcal{E}(i)
=
\{e\in\{1,\ldots,N\}:x_i\in\overline{\mathcal{K}}_e\}.
$$ {#eq-c5-incident-elements}

On the three-element mesh in @fig-c5-local-global-numbering,
$\mathcal{E}(1)=\{1\}$, $\mathcal{E}(2)=\{1,2\}$, and
$\mathcal{E}(3)=\{2,3\}$. This notation will be useful when element
contributions are collected at a global node.
...
More precisely, the support of $\phi_i$ is the closure of the set on which it
is nonzero. For the linear nodal basis,
...
$\mathcal{K}_e=[x_e,x_{e+1}]$, denote the coordinates of its local nodes by
...
Define two local shape functions
...
Let $U_a^e$ denote the displacement degree of freedom at local node $a$ of
element $e$:
...
$U_1^e$ and $U_2^e$. The connectivity map relates the local and global
coefficients:

$$
U_a^e=U_{I^e(a)},
\qquad a=1,2.
$$ {#eq-c5-local-global-dof}

For the consecutive numbering in @eq-c5-linear-connectivity,
$U_1^e=U_e$ and $U_2^e=U_{e+1}$. Differentiating the element interpolation
therefore gives
...
@eq-c5-global-entries already defines the
...
F_i^{\mathrm{dist}}
=
\int_0^L f\phi_i\,\dd x
=
\sum_{e=1}^{N}\int_{\mathcal{K}_e}f\phi_i\,\dd x.
$$ {#eq-c5-global-load-sum}
...
If $e\notin\mathcal{E}(i)$, then $\phi_i$ vanishes on $\mathcal{K}_e$, so that
element contributes zero. If $e\in\mathcal{E}(i)$, the local number of global
node $i$ is $J^e(i)$, and
$\phi_i|_{\mathcal{K}_e}=\phi_{J^e(i)}^e$. The global distributed-load entry is
therefore
...
$$ {#eq-c5-assembled-load-entry}

The complete global entry also contains the prescribed end load:

$$
F_i=F_i^{\mathrm{dist}}+\overline P\phi_i(L).
$$ {#eq-c5-complete-load-entry}
...
$\bK^e$ and $\bF^e$ belong together. In implementation, one element at a time
...
\qquad
K_{14}=0.
$$ {#eq-c5-assembly-entry-examples}

The first two entries receive contributions from both elements incident on
node 2. The entry $K_{23}$ receives one contribution because nodes 2 and 3
share only element 2, while $K_{14}$ is zero because the supports of
$\phi_1$ and $\phi_4$ do not overlap.
...
::: {.remark}
## Two equivalent views of load assembly

@eq-c5-assembled-load-entry fixes a global node $i$ and collects the local
entry from every element in $\mathcal{E}(i)$; the inverse map $J^e$ identifies
the required local node on each element. In an implementation, one normally
uses the equivalent forward operation: compute $\bF^e$, loop over its local
entries $F_a^e$, and add each entry to the global position $I^e(a)$ according
to @eq-c5-assembly-rule. Both views produce the global vector in
@eq-c5-three-element-F.
:::
